...
public class VarOrderHeap implements IOrder, Serializable {
	...
	protected static final double IMPORTANT_VARIABLES_BOUND = 2 * VAR_RESCALE_BOUND;
	...
	protected IVecInt fixedOrder = new VecInt();
	...
	private void varRescaleActivity() {
		for (int i = 1; i < activity.length; i++) {
			if (activity[i] < IMPORTANT_VARIABLES_BOUND) {
	...
	public void init() {
		...
		for (int i = 0; i < fixedOrder.size(); i++) {
			activity[fixedOrder.get(i)] = IMPORTANT_VARIABLES_BOUND;
		}
		for (int i = 1; i < nlength; i++) {
			assert i > 0;
